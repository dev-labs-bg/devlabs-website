 ...
 * version 1.0
 */ (function (a) {
    a.fn.scrollParallax = function (c) {
        var d = {
            speed: 0.2,
            axis: "x,y",
            debug: false
        ...
        function b(e) {
            if (d.debug && "console" in window && "log" in window.console) {
                console.log(e)
            }
        }
        return this.each(function () {
            ...
            h.bind("inview", function (i, j) {
                if (j == true) {
                    h.addClass("inview");
                    b("in view")
                } else {
                    h.removeClass("inview");
                    b("out of view")
                }
            });
            var f = h.css("backgroundPosition").split(" ");
            var g = parseInt(f[0].replace(/[^0-9\-]/g, ""));
            var e = parseInt(f[1].replace(/[^0-9\-]/g, ""));
            a(window).bind("scroll", function () {
                if (h.hasClass("inview")) {
                    var l = h.offset();
                    if (d.axis.match(/x/)) {
                        var m = l.left - a(window).scrollLeft();
                        var k = (-(m) * d.speed) + g
                    } else {
                        var k = g
                    } if (d.axis.match(/y/)) {
                        var j = l.top - a(window).scrollTop();
                        var i = (-(j) * d.speed) + e
                    } else {
                        var i = e
                    }
                    ...
                    b("new Y position: " + i);
                    h.css({
                        backgroundPositionY:  parseInt(i) + "px"
                    })
                }
            })
        })
